...
public class SetingsPanel extends JPanel implements observer{
	...
	private String[] prof = {"3", "4", "5", "6"};

	private JComboBox<String> selectDepth;
	private JComboBox<String> selectInputs;
	...
	public SetingsPanel(controller ctlr) {
		this.ctrl = ctlr;
		this.setPreferredSize(new Dimension(180,760));
		this.setMinimumSize(new Dimension(180,760));
		this.setMaximumSize(new Dimension(180,760));
	...
	private void initGUI() {
		...
		initPanel.setPreferredSize(new Dimension(180, 200));
		initPanel.setMinimumSize(new Dimension(180, 200));
		initPanel.setMaximumSize(new Dimension(180, 200));		 
		...
		init3 = new JPanel();
		JLabel lInit3 = new JLabel("Inputs");
		setDimLabel(lInit3, dim2);
		init3.add(lInit3);		 
		changeInput(entradas);
		initPanel.add(init3);

		init5 = new JPanel();
		JLabel lInit5 = new JLabel("Depth");
		setDimLabel(lInit5, dim2);
		init5.add(lInit5);		 
		changeDepth(prof);
		initPanel.add(init5);
	...
	private void changeInput(String[] prof2) {
		if(selectInputs != null)
			init3.remove(selectMut);
		selectInputs = new JComboBox<String>(prof2);
		selectInputs.setEditable(false);
		setNumVars(prof2[0]);
		selectInputs.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				setNumVars((String)selectInputs.getSelectedItem());
			}
		});
		setDimCombobox(selectInputs, dim2);
		init3.add(selectInputs);
		init3.validate();
		init3.repaint();
		this.repaint();
	}
	...
	private void changeDepth(String[] selec) {
		if(selectDepth!=null)
			init5.remove(selectDepth);
		selectDepth = new JComboBox<String>(selec);
		selectDepth.setEditable(false);
		seleccionarProfundidad(selec[0]);
		selectDepth.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				seleccionarProfundidad((String)selectDepth.getSelectedItem());
			}
		});
		setDimCombobox(selectDepth, dim2);
		init5.add(selectDepth);
		init5.validate();
		init5.repaint();
		this.repaint();
	}
	...
	protected void seleccionarProfundidad(String prof) {
		//ctrl.setInputs((int) prof);
	}
	...
	@Override
	public void onFinished(int bestFitness, double[][] best, double[][] bestGen, double[][] average, String bestResult,
